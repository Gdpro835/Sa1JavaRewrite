...
class SeabedVolcanoPlatform extends GimmickObject {
   ...
   public static boolean isShotting() {
      boolean var0;
      if (GameTime.event(SeabedVolcanoPlatform.class, "endCount", "isShotting:33", moving && GameTime.crosses(SeabedVolcanoPlatform.class, "endCount", endCount, 3))) {
         var0 = true;
      } else {
         var0 = false;
      }
   ...
   public static void shot() {
      moving = true;
      velocity = -1400;
      endCount = GameTime.set(SeabedVolcanoPlatform.class, "endCount", 3);
   }

   public static void staticLogic() {
      if (moving) {
         velocity = GameTime.advance(SeabedVolcanoPlatform.class, "velocity", velocity, GRAVITY);
         sPosY = GameTime.advance(SeabedVolcanoPlatform.class, "sPosY", sPosY, velocity);
         if (sPosY >= 0) {
            sPosY = 0;
            endCount = GameTime.advance(SeabedVolcanoPlatform.class, "endCount", endCount, -(1));
            if (endCount <= 0) {
               moving = false;
            } else {
               velocity = -velocity / 2;
